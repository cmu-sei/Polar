{ apiVersion = "apps/v1"
, kind = "StatefulSet"
, metadata = { name = "polar-neo4j", namespace = "polar-db" }
, spec =
  { replicas = 1
  , selector.matchLabels.name = "polar-neo4j"
  , serviceName = "polar-db-svc"
  , template =
    { metadata = { labels.name = "polar-neo4j", name = "polar-neo4j" }
    , spec =
      { containers =
        [ { env =
            [ { name = "NEO4J_AUTH"
              , valueFrom.secretKeyRef
                =
                { key = "NEO4J_AUTH", name = "neo4j-secret" }
              }
            ]
          , image = "neo4j:5.26.2"
          , name = "polar-neo4j"
          , ports = [ { containerPort = 7473 }, { containerPort = 7687 } ]
          , securityContext =
          ...
          , volumeMounts =
            [ { mountPath = "/var/lib/neo4j/conf", name = "neo4j-config-copy" }
            , { mountPath = "/var/lib/neo4j/data", name = "polar-db-data" }
            , { mountPath = "/var/lib/neo4j/logs", name = "polar-db-logs" }
            ]
          }
        ]
      , initContainers =
        [ { args =
          ...
          , volumeMounts =
            [ { mountPath = "/config", name = "neo4j-config" }
            , { mountPath = "/var/lib/neo4j/conf", name = "neo4j-config-copy" }
            ]
          }
        ]
      , securityContext =
        { fsGroup = 7474, fsGroupChangePolicy = "OnRootMismatch" }
      , volumes =
        [ { configMap =
              None { items : List { key : Text, path : Text }, name : Text }
          , emptyDir = None {}
          , name = "polar-db-data"
          , persistentVolumeClaim = Some { claimName = "polar-db-data" }
          }
        , { configMap =
              None { items : List { key : Text, path : Text }, name : Text }
          , emptyDir = None {}
          , name = "polar-db-logs"
          , persistentVolumeClaim = Some { claimName = "polar-db-logs" }
          }
        , { configMap = Some
            { items = [ { key = "neo4j.conf", path = "neo4j.conf" } ]
            , name = "neo4j-config"
            }
          , emptyDir = None {}
          , name = "neo4j-config"
