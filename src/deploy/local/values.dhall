{-
  ...
  - Certs are now issued at pod startup
...
let img = \(name : Text) -> "registry.local-registry.svc.cluster.local:5000/polar/${name}:${commitSha}"

let nuInitImage = img "polar-nu-init"
...
let jaegerDNSName =
      "http://${jaeger.service.name}.${C.polarNamespace}.svc.cluster.local:${Natural/show
