...
impl Actor for JiraIssueConsumer {
    ...
    async fn handle(
        ...
        fn merge_user(cypher: &mut String, role: &str, field: Option<&FieldValue>) {
                            ...
                            match value {
                                    ...
                                    for (i, item) in v.iter().enumerate() {
                                        match item {
                                            ...
                                            NestedListTypes::Object(the_item) => {
                                                ...
                                                } else if let Some(FirstTierField::Option(Some(
                                                    id,
                                                ))) = the_item.get("id")
                                                {
                                                    sub_key.push_str(id);
                                ...
                                FieldValue::Object(v) => {
                                    if key == "Assignee" {
                                        ...
                                        if let Some(FirstTierField::Option(Some(display))) =
                                            v.get("name")
                                        {
                                            second_cypher
                                                .push_str("MERGE (t:JiraIssueType {name: \"");
                                        ...
                                        if let Some(FirstTierField::Option(Some(parent_key))) =
                                            v.get("name")
                                        {
                                            second_cypher.push_str(&format!(
                                        ...
                                        {
                                            sub_key.push_str(name);
                                        } else if let Some(FirstTierField::Option(Some(id))) =
                                            v.get("id")
                                        {
                                            sub_key.push_str(id);
                                        } else {
                    ...
                    for base_item in issue.changelog.histories {
                        ...
                        for item in base_item.items {
                            ...
                            if let Some(option) = item.from {
                                let new_field_key = format!("field{field_count}");
                                field_count += 1;
                                params.insert(new_field_key.clone(), option);
                                changelogs
                                    .push_str(&format!(", cl{}.from=${new_field_key}", counter));
                            }
                            if let Some(option) = item.from_string {
                                // renamed field
                                let new_field_key = format!("field{field_count}");
                                field_count += 1;
                                params.insert(new_field_key.clone(), option);
                            ...
                            if let Some(option) = item.to {
                                ...
                                params.insert(new_field_key.clone(), option);
                                changelogs
                                    .push_str(&format!(", cl{}.to=${new_field_key}", counter));
                            }
                            if let Some(option) = item.to_string {
                                // renamed field
                                let new_field_key = format!("field{field_count}");
                                field_count += 1;
                                params.insert(new_field_key.clone(), option);
                            ...
                            changelogs
                                .push_str(
                                    &format!("MERGE (i)-[:Transitioned]->(cl{})\n", counter,),
                                );
                            counter += 1;
                        }
                    }
