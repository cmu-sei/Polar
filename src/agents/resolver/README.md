...
- Emit "resolved" events to the broker, which are then consumed by the `build-processor` agent to link container images to SBOMs, build pipelines, commits, and software components.
