...
-- openapi/processor.dhall  (same as jira/processor pattern)
let kubernetes = ../../../../schema/kubernetes.dhall
let Constants  = ../../../../schema/constants.dhall
let functions  = ../../../../schema/functions.dhall

let render =
      \(v : { name : Text, image : Text, imagePullPolicy : Text, imagePullSecrets : List { name : Optional Text }, certClientImage : Text, certIssuerUrl : Text, saTokenAudience : Text, neo4jBoltAddr : Text }) ->
        let volumes = [ Constants.certEmptyDirVolume, Constants.saTokenVolume v.saTokenAudience, kubernetes.Volume::{ name = "neo4j-bolt-ca", configMap = Some kubernetes.ConfigMapVolumeSource::{ name = Some "neo4j-bolt-ca" } } ]
        ...
        let mounts = [ Constants.certVolumeMount, kubernetes.VolumeMount::{ name = "neo4j-bolt-ca", mountPath = "/etc/neo4j-ca", readOnly = Some True } ]
