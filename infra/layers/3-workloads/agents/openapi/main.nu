...
def main [context_nuon: string] {
    ...
    let cert_args   = ", polarInitImage = \"" + $polar_init_image + "\", certIssuerUrl = \"" + $cert_issuer_url + "\", saTokenAudience = \"" + $sa_audience + "\" }"
    ...
    dhall-to-yaml --documents --quoted --file $tmp | save --force ($output_dir | path join "openapi-observer.yaml")
    rm $tmp
    ...
    dhall-to-yaml --documents --quoted --file $tmp | save --force ($output_dir | path join "openapi-processor.yaml")
    rm $tmp
