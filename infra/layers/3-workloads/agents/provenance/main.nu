...
def main [context_nuon: string] {
    ...
    dhall-to-yaml --documents --quoted --file $tmp | save --force ($output_dir | path join "provenance-linker.yaml")
    rm $tmp
    ...
    dhall-to-yaml --documents --quoted --file $tmp | save --force ($output_dir | path join "provenance-resolver.yaml")
    rm $tmp
