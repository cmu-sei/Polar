-- infra/layers/3-workloads/agents/provenance/linker.dhall
...
let render =
      \(v :
        ...
        in  kubernetes.Deployment::{
            , metadata = kubernetes.ObjectMeta::{ name = Some Constants.ArtifactLinkerName, namespace = Some Constants.PolarNamespace, annotations = Some [ Constants.RejectSidecarAnnotation ] }
            , spec = Some kubernetes.DeploymentSpec::{
              , selector = kubernetes.LabelSelector::{ matchLabels = Some (toMap { name = Constants.ArtifactLinkerName }) }
              , replicas = Some 1
              , template = kubernetes.PodTemplateSpec::{
                , metadata = Some kubernetes.ObjectMeta::{ name = Some Constants.ArtifactLinkerName, labels = Some [ { mapKey = "name", mapValue = Constants.ArtifactLinkerName } ] }
