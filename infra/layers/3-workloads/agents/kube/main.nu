...
def main [context_nuon: string] {
    ...
    dhall-to-yaml --quoted --documents --file $tmp | save --force ($output_dir | path join "kube-agent-rbac.yaml")
    rm $tmp
    let tmp  = (mktemp --suffix ".dhall")
    ...
    dhall-to-yaml --quoted --documents --file $tmp | save --force ($output_dir | path join "kube-observer.yaml")
    rm $tmp
    let tmp  = (mktemp --suffix ".dhall")
    ...
    dhall-to-yaml --quoted --documents --file $tmp | save --force ($output_dir | path join "kube-consumer.yaml")
    rm $tmp
    print $"  kube: done"
}
