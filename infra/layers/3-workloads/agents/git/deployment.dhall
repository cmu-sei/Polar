-- infra/layers/3-workloads/agents/git/deployment.dhall
--
-- Git agent Deployment.
-- Three containers: observer, consumer, scheduler.
-- Observer config (git.json) is mounted from a Secret.
...
let render =
      \(v :
          ...
          , observer   : { name : Text, image : Text }
          , consumer   : { name : Text, image : Text }
          , scheduler  : { name : Text, image : Text }
          , tls :
        ...
        let volumes =
              [ tlsVolume, configVolume, neo4jCAVolume ]
                ...
                , spec = Some kubernetes.PodSpec::{
                  ...
                  , containers =
                    [ kubernetes.Container::{
                      ...
                      , volumeMounts    = Some (baseMounts # [ configMount ])
                      }
                    , kubernetes.Container::{
                    ...
                    , kubernetes.Container::{
                      , name            = v.scheduler.name
                      , image           = Some v.scheduler.image
                      , imagePullPolicy = Some v.imagePullPolicy
                      , securityContext = Some Constants.DropAllCapSecurityContext
                      , env             = Some schedulerEnv
                      , volumeMounts    = Some (baseMounts # [ neo4jCAMount ])
                      }
                    ]
                  }
